...
parser.add_argument('-sd', '--start_date', type=str, help="start date from which results are calculated")
parser.add_argument('-ed', '--end_date', type=str, help="end date to which results are calculated")
parser.add_argument('-ni', '--night', action='store_true', help="Argument for isolating last 10 results for each model")
parser.add_argument('-ep', '--excel_path', type=str, help="Excel path where target excel file is located")
parser.add_argument('-rp', '--results_path', type=str, required=True, help="Path to test results")
...
if args.night and args.start_date:
    sys.exit("You can not combine dats and night arguments")
#this checks if excel path is provided when night argument is provided, that is mandatory
if (args.night and not args.excel_path) or (not args.night and args.excel_path):
...
def get_last_file_in_excel_path():
    new_path = sorted(pathlib.Path(args.excel_path).glob('*xlsx'), key=os.path.getmtime)[-1]
    return str(new_path)
...
def get_folders_between_dates():
    ...
    for member in os.listdir(args.results_path):
        element = os.path.join(args.results_path, member)
...
def get_last_x_folders(x):
    ...
    while temp <= x:
        counter += 1
        path = str(get_last_nth(counter, args.results_path))
...
#this functions calls the functions that returns last 10 folders and then it colects all resulrs from each model into a dictionary
...
def link_main_table_to_model_sheets():
    wb = load_workbook(get_last_file_in_excel_path())
    target_doc_basename = os.path.basename(get_last_file_in_excel_path())
    ...
    while counter < target_sheet.max_row:
        ...
        counter += 1
    wb.save(get_last_file_in_excel_path())


#adds functionality of go back button
def go_back_button(target_sheet):
    main_doc_basename = os.path.basename(get_last_file_in_excel_path())
